...
wp_embed_register_handler( 'instagram', '/https?\:\/\/instagram.com\/p\/(.+)/', 'eif_instagram_embed_function' );
...
function eif_instagram_embed_function( $matches, $attr, $url, $rawattr ) {
    ...
    $embed = sprintf(
            '
<iframe src="//instagram.com/p/%1$s/embed/" class="embed_css frameborder="0" height="673" data-instgrm-payload-id="instagram-media-payload-0" scrolling="no" style="border: 0px; margin: 1px; border-radius: 4px;  box-shadow: rgba(0, 0, 0, 0.498039) 0px 0px 1px 0px, rgba(0, 0, 0, 0.14902) 0px 1px 10px 0px; display: block; padding: 0px; background: rgb(255, 255, 255);"></iframe>',
            esc_attr($image_id)
      
            );
?>
  <style type="text/css">
        iframe.embed_css { max-width: 658px; width: calc(100% - 2px); }
iframe { position: relative !important; }
 </style><?php
    return apply_filters( 'embed_instagram', $embed, $matches, $attr, $url, $rawattr );
}
